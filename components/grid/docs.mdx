...
import { Box, GridOverlay, tuples } from './examples/_utils';

# Grid

It’s very important to pay attention to the layout and spacing of every element
	...
	<GridOverlay>
		<Grid style={{ fontSize: 14 }}>
			<Cell width={[12, 10]} left={[1, 2]}>
				<Box>Offset Column</Box>
			</Cell>
			<Cell width={[12, 5]} left={[1, 7]}>
				<Box>Offset Column</Box>
			</Cell>
		</Grid>
	</GridOverlay>
...
<Playground>
	<GridOverlay columns={16} gap={16}>
		<Grid style={{ fontSize: 14 }} columns={16} gap={16}>
			<Cell width={[16, 8, 2, 8]}>
				<Box>Column A</Box>
			</Cell>
			<Cell width={[16, 8, 6, 4]}>
